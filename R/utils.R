


#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# each index from 0-255 returns
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
reverse_bit_lookup <- as.raw(
  c())




#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#' Pad a logical vector to the given size
#' 
#' @param bits logical vector
#' @param nbits Pad width to a multiple of this number of bits
#' @param side 'left' or 'right'.  Only the lowercase version of the
#'     first letter is used to determine the side. all of these are valid 
#'     options: 'L', 'R', 'left', 'Right'
#' @param value The value to use for padding. single logical value. 
#'     Default: FALSE
#' @return Logical vector with the specified padding
#' @examples
#' pad_bits(c(TRUE, TRUE), nbits = 8, side = 'left')
#' pad_bits(c(TRUE, TRUE), nbits = 8, side = 'R')
#' 
#' @export
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
pad_bits <- function(bits, nbits = 8L, side = 'left', value = FALSE) {
  
  stopifnot(is.logical(bits))
  if (length(bits) %% nbits == 0) {
    return(bits)
  }
  
  n_extra_bits <- nbits - (length(bits) %% nbits)
  padding <- rep(isTRUE(value), n_extra_bits)
  
  side <- tolower(side)
  if (startsWith(side, 'l')) {
    c(padding, bits)
  } else if (startsWith(side, 'r')) {
    c(bits, padding)
  } else {
    stop("pad_bits(): Unknown padding side = ", side);
  }
}


